...
public final class AppendingMediaSource implements MediaSource {
  ...
  public AppendingMediaSource(MediaSource... mediaSources) {
    ...
    Collections.addAll(this.mediaSources, mediaSources);
  ...
  @Override public MediaPeriod createPeriod(int index, Allocator allocator, long positionUs) {
    ...
    MediaPeriod mediaPeriod =
        mediaSources.get(index).createPeriod(periodIndexInSource, allocator, positionUs);
  ...
  private static SparseArray<Boolean> buildDuplicateFlags(List<MediaSource> mediaSources) {
    ...
    IdentityHashMap<MediaSource, Void> sources =
        new IdentityHashMap<MediaSource, Void>(mediaSources.size());
    for (int i = 0; i < mediaSources.size(); i++) {
  ...
  private static final class AppendingTimeline extends Timeline {
    ...
    @Override public Window getWindow(int windowIndex, Window window, boolean setIds) {
    ...
    @Override public Window getWindow(int windowIndex, Window window, boolean setIds,
      ...
      return getWindow(windowIndex, window, setIds);
    }
  ...
  public void appendSource(ExoPlayer player, MediaSource mediaSource) {
    ...
    if (!mediaSources.isEmpty()) {
      prepareSource(player, mediaSources.size() - 1);
    } else {
      prepareSource(player, 0);
    }
  }

  public void appendSource(ExoPlayer player, String uriString, Context mContext) {
    appendSource(player, buildMediaSource(uriString, mContext));
  }

  private void prepareSource(ExoPlayer player, final int index) {

    mediaSources.get(index).prepareSource(player, false, new Listener() {
      @Override public void onSourceInfoRefreshed(Timeline timeline, Object manifest) {
        handleSourceInfoRefreshed(index, timeline, manifest);
      }
    });
  ...
  private MediaSource buildMediaSource(String uriString, Context mContext) {
    ...
    return new ExtractorMediaSource(Uri.parse(uriString), dataSourceFactory, extractorsFactory,
        null, null);
  }
